...
def n_beats_model_builder(hp):
    input_dim = hp.Choice('input_dim', [32, 64, 128])
...
def setup_neural_network_tuning(model_builder, X_train, y_train, X_val, y_val):
    ...
    tuner = RandomSearch(
        model_builder,
        objective='val_loss',
        max_trials=1,  # Adjust as necessary
    ...
    tuner.search(X_train, y_train, epochs=1, validation_data=(X_val, y_val))  # Adjust epochs as necessary
